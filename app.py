...
st.set_page_config(page_title="Data Analysis", layout="wide")import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
from io import BytesIO
...
if uploaded_file is not None:
    ...
    with col1:
        ...
        with st.spinner('Loading chart...'):
            fig, ax = plt.subplots(figsize=(8,5))
            ax = sns.countplot(data=filtered_df, x='Shaadi')
            for bars in ax.containers:
                ax.bar_label(bars)
            st.pyplot(fig)
    ...
    with col2:
        ...
        with st.spinner('Loading chart...'):
            ...
            fig, ax = plt.subplots(figsize=(8,5))
            sns.barplot(data=sales_state, x='Shaadi', y='Amount', hue='Gender', ax=ax)
            st.pyplot(fig)
    ...
    with col1:
        ...
        with st.spinner('Loading chart...'):
            ...
            ax = sns.countplot(data=filtered_df, x='Occupation')
            for bars in ax.containers:
                ax.bar_label(bars)
            plt.xticks(rotation=90)
            st.pyplot(fig)
    ...
    with col1:
        ...
        with st.spinner('Loading chart...'):
            fig, ax = plt.subplots(figsize=(10,6))
            ax = sns.countplot(data=filtered_df, x='Product_Category')
            for bars in ax.containers:
                ax.bar_label(bars)
            plt.xticks(rotation=90)
            st.pyplot(fig)
    ...
    with col1:
        ...
        with st.spinner('Loading chart...'):
            ...
            fig, ax = plt.subplots(figsize=(10,6))
            sns.barplot(data=sales_state, x='Product_ID', y='Orders', ax=ax)
            plt.xticks(rotation=90)
            st.pyplot(fig)
...
if uploaded_file is not None:
    ...
    with col2:
        ...
        with st.spinner('Loading chart...'):
            fig, ax = plt.subplots(figsize=(10,6))
            filtered_df.groupby('Product_ID')['Orders'].sum().nlargest(10).sort_values(ascending=False).plot(kind='bar', ax=ax, color=sns.color_palette('Set2', n_colors=10))
            plt.xticks(rotation=90)
            st.pyplot(fig)
            download_button(fig, "top_products_alternative.png")
